...
class Matchup:
    ...
    cost_usd: float
    tokens: int
    candidate_output_tokens: int = 0  # tokens the candidate itself generated
    incumbent_output_tokens: int = 0
    transcript: str = ""


def _side_output_tokens(meter, side: str) -> int:
    """Output tokens generated by one side's advocate in a debate."""
    return sum(
        e.output_tokens for e in meter.events if e.module == f"debate.{side}"
    )
...
def run_eval(
    ...
    for topic in topics:
        for side in ("prosecution", "defense"):
            ...
            incumbent_side = "defense" if side == "prosecution" else "prosecution"
            try:
                ...
                matchup = Matchup(
                    ...
                    candidate_output_tokens=_side_output_tokens(outcome.usage, side),
                    incumbent_output_tokens=_side_output_tokens(outcome.usage, incumbent_side),
            ...
            log(f"  [{done}/{total}] {matchup.topic} (candidate as {side}): "
                ...
                f"{matchup.tokens:,} tok, cand out {matchup.candidate_output_tokens:,} "
                f"vs inc {matchup.incumbent_output_tokens:,}, ${matchup.cost_usd:.4f})")
            if on_progress:
    ...
    # Verboseness: how many tokens each side spends making its case, and what
    # a win costs in tokens. Ratio > 1 means the candidate argues longer than
    # the incumbent for its results.
    cand_out = sum(m.candidate_output_tokens for m in matchups)
    inc_out = sum(m.incumbent_output_tokens for m in matchups)
    verbosity = {
        "candidate_output_tokens": cand_out,
        "incumbent_output_tokens": inc_out,
        "ratio": round(cand_out / inc_out, 2) if inc_out else None,
        "candidate_tokens_per_win": round(cand_out / wins) if wins else None,
        "incumbent_tokens_per_win": round(inc_out / losses) if losses else None,
    }
...
def _verbosity_note(verbosity: dict) -> str:
    """One-line verboseness summary for reports and findings."""
    if not verbosity or verbosity.get("ratio") is None:
        return "Verbosity: n/a."
    note = f"Verbosity {verbosity['ratio']}x vs incumbent"
    if verbosity.get("candidate_tokens_per_win"):
        note += f"; {verbosity['candidate_tokens_per_win']:,} output tokens per win"
    return note + "."
...
def to_apa_finding(report: dict) -> dict:
    ...
    return {
        ...
        "why": (
            ...
            f"round(s) each). {_verbosity_note(report.get('verbosity', {}))} "
            f"Eval cost ${report['total_cost_usd']}."
        ),
...
def main():
    ...
    print(f"  {_verbosity_note(report['verbosity'])}")
    v = report["verbosity"]
    print(f"  Output tokens: candidate {v['candidate_output_tokens']:,} "
          f"vs incumbent {v['incumbent_output_tokens']:,}")
